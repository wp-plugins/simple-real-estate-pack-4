...
function srp_tabs_byType($args = array(), $ajax = NULL){
	...
	if(!$types = srp_groupSchoolsBy($args)) { return; }
	$html_arr = array();
	...
	foreach($types as $type){				
		...
		if($distance = $args['location']['distance']){			
			$$args['output'] .= "<p>$total $name Schools found within $distance miles radius.</p>\n";
		}else{
			$$args['output'] .= "<p>$total $name Schools found in the area.</p>\n";
		}
		$list .= "\t<ul>\n";
		...
		foreach($type as $item){
			...
			$coordinates[$schoolname]['html'] = '<div><span class="school_name">'.$school->schoolname.'</span><br />'.$school->schooltype . ' - ' . $school->gradesserved .'<br />Phone: '.$school->phonenumber.'<br />'.$school->address.', '. $school->city.', '.$school->state.' '. $school->zip.'</div>';
			$x++;
		}
		$list .= "\t<ul>\n";
		$table .= "</table>\n";
