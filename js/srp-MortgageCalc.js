// JavaScript Document
jQuery(function() {
		jQuery(".srp-tabs").tabs();	
		});

function srp_ajax_loaderStart(title, id){	
	...
	var loading_message = '<div id="ajax_loading">' + img + title + '</div>';
	//alert(message);
	jQuery("#gre_map_canvas").append(loading_message);
	var ajx_w = jQuery("#ajax_loading").width();
	var ajx_h = jQuery("#ajax_loading").height();
	var x = width/2 - ajx_w/2;
	var y = height/2 - ajx_h/2;
	jQuery("#ajax_loading").css({'top' : y, 'left' : x});
	
	window.setTimeout(function() {
		 srp_ajax_loaderStop();
	}, 5000);
}
function srp_ajax_loaderStop(){
